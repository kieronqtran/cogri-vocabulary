...
exports.run = async (browser, context) => {
  ...
  try {
    ...
    await pageOne.goto('https://dictionary.cambridge.org/dictionary/english-vietnamese', { waitUntil: ['domcontentloaded'] });
    ...
    //   return true;
    // });
    await Promise.all([pageOne.waitForNavigation()]);

    await pageOne.waitFor('span.trans').catch(err => {});
    const elementsOne = await pageOne.$$('span.def-body').catch(err => {});
    ...
    if (elementsOne != null) {
      if (elementsOne[0] != null) {
        ...
        const transOne = await firstElementOne.$eval('span.trans', element => element.innerHTML).catch(err => console.log(err));

        if (transOne != null) {
          vietnameseMeaning = transOne.trim();
          vietnameseMeaning = vietnameseMeaning.replace(/<\/?[^>]+(>|$)/g, '');
        }

        const rawExampleOne = await firstElementOne.$eval('div.examp.emphasized > span.eg', element => element.innerHTML).catch(err => console.log(err));

        let sampleSentenceOne = '';
        if (rawExampleOne != null) {
          sampleSentenceOne = rawExampleOne.replace(/<\/?[^>]+(>|$)/g, '');
          sampleSentenceOne = sampleSentenceOne.replace(/[^a-zA-Z0-9]/g, " ");
      ...
      if (elementsOne[1] != null) {
        ...
        const rawExampleTwo = await secondElementOne.$eval('div.examp.emphasized > span.eg', element => element.innerHTML).catch(err => console.log(err));

        let sampleSentenceTwo = "";
        if (rawExampleTwo != null) {
          sampleSentenceTwo = rawExampleTwo.replace(/<\/?[^>]+(>|$)/g, '');
          sampleSentenceTwo = sampleSentenceTwo.replace(/[^a-zA-Z0-9]/g, " ");
    ...
    */
    ...
    await pageTwo.goto('http://www.synonym-finder.com', { waitUntil: ['domcontentloaded'] });

    await pageTwo.type('#edit-name', word);
    await pageTwo.keyboard.press('Enter');
    await Promise.all([pageTwo.waitForNavigation()]);

    let similar = [];

    // await pageTwo.screenshot({path: '/tmp/screenshot.png'});
    const found = await pageTwo.waitFor('span.hypernym-lemmas').catch(error => {});

    if (found != null) {
      const elementsTwo = await pageTwo.$$('span.hypernym-lemmas').catch(error => {});

      if (elementsTwo != null) {
        const firstElementTwo = elementsTwo[0];
        let synonym = await firstElementTwo.$eval('a', element => element.innerHTML).catch(error => {});

        if (synonym != null) {
          synonym = synonym.replace(/['"]+/g, ' ');
          synonym = synonym.replace(/[^a-zA-Z0-9]/g, " ");
    ...
    if (vietnameseMeaning != '') {
      ...
      const projectId = 'cogri-vocabulary';
      ...
      return translate.translate(text, target).then(async results => {
